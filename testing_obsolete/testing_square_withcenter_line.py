...
for i,batch in enumerate(test_loader):
    ...
    for component_no in range(len(ids)):
        ...
        #current score map is skeletonized version of each component of max scale//4
    ...
    for i,center_line_mask in enumerate(component_score_maps):
        ...
        #print("variance map shape",variance_map.shape)
    #print("godzilla")
    print(len(sample_contours),len(component_score_maps),len(area_list),len(center_line_list))
    #print("bageshri",sample_contours[0].shape,center_line_list[0].shape)
    filtered_contour_list=sample_contours
    filtered_center_line_list= center_line_list
    
    #scale the filtered gaussians 
