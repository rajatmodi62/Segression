...
def variance_filter(variance_map, max_value_index, mask):
    # max_value_index : hxw
    ...
    print('INTERMEDIATE VALUES ==>', variance_map.shape, max_value_index.shape)
    print(max_value_index)
    non_zero_index = mask.nonzero()
    for index in non_zero_index:
        filtered_variance_map[index[0],:,index[1],index[2]]= variance_map[index[0],:,index[1],index[2]]
        filtered_variance_map[index[0],0,index[1],index[2]]=filtered_variance_map[index[0],0,index[1],index[2]]*scaling_factor[index[0]]
        filtered_variance_map[index[0],1,index[1],index[2]]=filtered_variance_map[index[0],1,index[1],index[2]]*scaling_factor[index[0]]
...
for i,batch in enumerate(test_loader):
    ...
    # plt.show()
    variance_map = variance_filter(pred_variance_map, max_value_index, score_map)
    score_map = score_map.detach().cpu().numpy()
    score_map= (score_map>0).astype('uint8')
