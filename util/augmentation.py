...
class RandomResizedLimitCrop_EAST(object):
	def __init__(self, size, scale=(0.3, 1.0), ratio=(3. / 4., 4. / 3.)):
		self.size = (size, size)
		self.scale = scale
		self.ratio = ratio
	# borrowed : https://github.com/SakuraRiven/EAST
	def is_cross_text(self, start_loc, length, polygons):
		...
		for polygon in polygons:
			p2 = Polygon(polygon.points.reshape((-1,2))).convex_hull
	...
	# borrowed : https://github.com/SakuraRiven/EAST
	def cropimg(self, img, polygons,  length):
		...
		for polygon in polygons:
			if polygon.points.size > 0:
				polygon.points[:,0] = polygon.points[:,0]*ratio_w
				polygon.points[:,1] = polygon.points[:,1]*ratio_h
		...
		while flag and cnt < 1000:
			...
			flag = self.is_cross_text([start_w, start_h], length, polygons)
		...
		for polygon in polygons:
			polygon.points[:,0] -= start_w
			polygon.points[:,1] -= start_h

		return region, polygons

	def __call__(self, image, polygons=None):
		#img, polygones = self.get_params(image, self.scale, self.ratio, polygons)
		# for polygon in polygons:
		# 	print(polygon.points)
		area = image.shape[0] * image.shape[1]
		target_area = np.random.uniform(*self.scale) * area
		aspect_ratio = np.random.uniform(*self.ratio)
		#print("rajat",target_area,aspect_ratio)
		w = int(round(math.sqrt(target_area * aspect_ratio)))
		h = int(round(math.sqrt(target_area / aspect_ratio)))
		if np.random.random() < 0.5:
			w, h = h, w
		#print("before",image.shape,'size item ', self.size[0])
		image = Image.fromarray(image.astype('uint8'), 'RGB')
		crop_region, new_polygons = self.cropimg(image, polygons,  length=self.size[0])
		crop_region = np.array(crop_region)
		return crop_region, new_polygons
