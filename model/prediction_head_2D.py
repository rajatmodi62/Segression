...
Input:
Variance Map: [N,2,H,W]
Segmentation Map: [N,2,H,W] tensor of 0/1's
...
'''
class PredictionHead2D(nn.Module):
    ...
    def __init__(self,\
        ...
        super(PredictionHead2D, self).__init__()
    ...
    def forward(self,\
        ...
        for i in range(batch_size):
            ...
            batch_pooled_gaussians= self.create_gaussian_2D(variance_height,\
                                    variance_width,\
                                    x_coordinate.float(),\
                                    y_coordinate.float(),\
                                    height,\
...
if __name__ == '__main__':
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    prediction_head=  PredictionHead2D()
    ...
    variance_map= torch.zeros((2,2,128,256))
